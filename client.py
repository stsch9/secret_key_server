...
headers = {"accept": "application/json",
           "Content-Type": "application/json;charset=UTF-8"}
data = {"derivation_salt": Base64Encoder.encode(raw_salt).decode('utf-8'),
        "signing_key": Base64Encoder.encode(raw_signing_key).decode('utf-8'),
        "encryption_key": Base64Encoder.encode(raw_encryption_key).decode('utf-8')}
response = requests.put("http://127.0.0.1:5000/api/key", auth=HmacAuth(1, raw_signing_key), data=json.dumps(data), headers=headers)
print(response.json())
